...
function checkActivate(obj) {
  // console.log(obj);
  let addCommentButton = obj.parentNode.parentNode.children[1];
  // console.log(addCommentButton);
  if(obj.value) {addCommentButton.classList.add("active");}
  else addCommentButton.classList.remove("active");
}
...
function activatePersonalProfileSubMenu() {
  ...
  if(personalProfileSubMenu.classList.value.includes('active')) personalProfileSubMenu.classList.remove('active');
  else personalProfileSubMenu.classList.add('active');
}
...
function liveSearchForProfile(e) {
  ...
  if (dataContainer.length >=1) dataContainer.forEach((data)=>createSearchProfileResult(...data));
  else createSearchProfileResult();
}

function createSearchProfileResult (imgUrl, userId, name) {
  console.log(imgUrl);
